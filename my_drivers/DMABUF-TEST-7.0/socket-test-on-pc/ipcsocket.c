...
int sendtosocket(int sockfd, struct socketdata *skdata)
{
	...
	//setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char *)&temp, sizeof(int));
	#if 0
	ret = listen(sockfd, 5);
	if (ret < 0) {
		fprintf(stderr,"ERROR: sendtosocket: listen - Failed <%s>\n",strerror(errno));
		return -1;
	}
	#endif
#if 0
	printf("sendtosocket: Doing : accept\n");
	memset(&skaddr, 0, sizeof(struct sockaddr_un));
	sklen = sizeof(skaddr);
	clientfd = accept(sockfd,(struct sockaddr *)&skaddr,(socklen_t *)&sklen);
	if (ret < 0) {
		fprintf(stderr,"ERROR: sendtosocket: accept - Failed <%s>\n",strerror(errno));
...
int receivefromsocket(int sockfd, struct socketdata *skdata)
{
	...
	if (ret < 0) {
		...
		#endif
		//fprintf(stderr,"ERROR: receivefromsocket: connect - Failed, errno: %d <%s>\n",errno,strerror(errno));
		//return -1;
	}
	#endif
	FD_ZERO(&recvFDs);
...
if(FD_ISSET(sockfd,&recvFDs))
{
	...
	ret = recv(sockfd, &buffd, len, MSG_DONTWAIT);
	if (ret < 0) {
